...
------------------------------------------------------------------
Features: BERT (Sliding Window) + Autoencoder + Isolation Forest + LIME + S-BERT + Rule Engine.
Author: Yoge-2004
Version: 15.1.0 (Sliding Window & Optimized XAI)
"""
...
COMMON_CORP_TERMS = {
    ...
    'coo', 'cmo', 'cio', 'ciso', 'cdo', 'chro', 'clo', 'cpo', 'cso', 'founder', 'owner',
...
log_event("Initializing JobGuard System v15.1...", "INIT", "BOOT")
    ...
    def extract_entities_robust(text: str) -> Dict[str, List[str]]:
        ...
        for pat in salary_patterns:
            ...
            for m in matches:
                ...
                if re.match(r'^(19|20)\d{2}$', clean_match): 
                    continue
...
class FraudDetector:
    ...
    def predict(self, text: str) -> Dict[str, Any]:
        ...
        def trace(msg, level="INFO"): 
            logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] [{level}] {msg}")
        ...
        if not is_valid:
            ...
            return {'status': 'invalid', 'warnings': warnings, 'logs': logs, 'feats_scaled': None}
        ...
        input_ids_full = model_mgr.bert_tokenizer.encode(text, add_special_tokens=False)
        if not input_ids_full: 
            input_ids_full = [0]
            
        chunk_size = 510  # Leave 2 spaces for [CLS] and [SEP]
        stride = 256
        
        chunk_probs = []
        chunk_cls_embs = []
        
        for i in range(0, max(1, len(input_ids_full)), stride):
            chunk = input_ids_full[i:i + chunk_size]
            chunk_with_special = [101] + chunk + [102]
            
            pad_len = 512 - len(chunk_with_special)
            chunk_padded = chunk_with_special + [0] * pad_len
            attn_mask = [1] * len(chunk_with_special) + [0] * pad_len
            
            chunk_tensor = torch.tensor([chunk_padded]).to(DEVICE)
            mask_tensor = torch.tensor([attn_mask]).to(DEVICE)
            feats_tensor = torch.tensor(feats_scaled, dtype=torch.float32).to(DEVICE)
            
            with torch.no_grad():
                logits = model_mgr.bert_fusion(chunk_tensor, mask_tensor, feats_tensor)
                chunk_probs.append(torch.sigmoid(logits).item())
                
                # Extract CLS for Autoencoder
                cls = model_mgr.bert_fusion.bert(input_ids=chunk_tensor, attention_mask=mask_tensor).last_hidden_state[:, 0, :].numpy()
                chunk_cls_embs.append(cls[0])

        # --- 4. TOP-K MEAN AGGREGATION ---
        k = min(3, len(chunk_probs))
        top_k_probs = sorted(chunk_probs, reverse=True)[:k]
        bert_prob = sum(top_k_probs) / k
        
        # Mean pool the embeddings for the unsupervised branch
        cls_emb = np.mean(chunk_cls_embs, axis=0).reshape(1, -1)
        
        ae_emb_scaled = model_mgr.scalers['ae'].transform(cls_emb)
        with tf.device('/cpu:0'): 
            recon = model_mgr.autoencoder.predict(ae_emb_scaled, verbose=0)
            ...
            if sentences:
                ...
                for i, sent_scores in enumerate(scores):
                    ...
                    if curr_max > 0.55:
                        ...
                        if 'interview' in suspicious_sent and any(sw in suspicious_sent for sw in safe_words): 
                            continue
        ...
        advisory = []
        if entities['emails']: 
            advisory.append(f"✅ **Email:** Verified ({' '.join(entities['emails'][:1])})")
        else: 
            advisory.append("ℹ️ **Email:** Not listed in description")
            
        if entities['urls']: 
            advisory.append(f"✅ **Website:** Link detected ({' '.join(entities['urls'][:1])})")
        else: 
            advisory.append("ℹ️ **Website:** Not listed in description")
            
        if entities['salaries']: 
            advisory.append(f"✅ **Compensation:** {' '.join(entities['salaries'][:1])}")
        else: 
            advisory.append("ℹ️ **Compensation:** Not explicitly mentioned")
        
        if entities['phones']: 
            advisory.append(f"✅ **Phone:** {' '.join(entities['phones'][:1])}")
        else: 
            advisory.append("ℹ️ **Phone:** Not listed in description")
...
def predict():
    try:
        ...
        if final_prob > 0.35:
            try:
                real_feats = result['feats_scaled']
                ...
                def batch_predict_lime(texts: List[str]) -> np.ndarray:
                    ...
                    for i in range(0, len(texts), batch_size):
                        ...
                        inputs = model_mgr.bert_tokenizer(batch_texts, return_tensors="pt", max_length=512, padding=True, truncation=True)
                        
                        actual_feats_batch = torch.tensor(np.repeat(real_feats, len(batch_texts), axis=0), dtype=torch.float32).to(DEVICE)
                        
                        with torch.no_grad():
                            logits = model_mgr.bert_fusion(inputs['input_ids'].to(DEVICE), inputs['attention_mask'].to(DEVICE), actual_feats_batch)
                ...
                exp = lime_explainer.explain_instance(text, batch_predict_lime, num_features=6, num_samples=30)
